...
| 채팅 타임아웃 | 런처 플래그 `--chat-timeout=<ms>`로 `CHAT_TIMEOUT_MS` 주입 |
...
| Web Push 권한 | Playwright context `permissions: ['notifications']` 주입 (런처 설정) |
...
| TERMINAL_ENABLED=false | 런처가 보조 인스턴스를 다른 포트에 띄워 해당 환경변수 주입 |
