...
**절차**:
1. BMad 프로젝트의 채팅 세션에서 **채팅 입력바 하단**의 BMad 에이전트 버튼 (`aria-label="BMad 에이전트 목록"`) 클릭
2. SM / PM / Architect / Dev / QA 순차 선택 → 각각 간단한 프롬프트 전송 ("Hello, who are you?")
3. `browser_snapshot`으로 활성 에이전트 배지 변경 확인
