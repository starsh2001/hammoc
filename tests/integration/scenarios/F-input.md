...
### F-04-03: 스니펫 팔레트 내용 미리보기 (인라인 확장)
**목적**: 스니펫 팔레트에서 각 항목의 펼침 버튼(▸ 쉐브론)을 눌러 전체 본문을 인라인으로 미리 볼 수 있는지 검증.

**선행 조건**: F-04-02 와 동일 — 테스트 프로젝트 경로에 `.hammoc/snippets/test-preview.md` 파일 생성 (본문 3줄 이상).

**절차**:
1. ChatInput에 `%` 입력 → 팔레트 오픈.
2. `test-preview` 항목 우측의 **쉐브론 버튼**(▸) 클릭 — 쉐브론이 회전(▾)하고 항목 아래에 **본문 전체가 모노스페이스 블록**으로 표시되는지 확인.
3. 표시된 본문이 파일 원문과 일치하는지 확인.
4. 같은 쉐브론 다시 클릭 → 본문 블록이 접혀 사라지는지 확인.
5. 다른 스니펫의 쉐브론 클릭 → 이전 확장이 접히고 새 스니펫이 펼쳐지는지 확인.
6. 펼침 상태에서 **항목 본체(이름/미리보기 영역)** 클릭 → 스니펫이 선택(삽입)되는지 확인 (펼침 버튼 클릭은 선택과 독립).

**기대 결과**:
- 쉐브론 클릭으로 서버에서 본문을 fetch해 인라인 표시 (최대 150px 스크롤 영역)
- 한 번 fetch한 내용은 캐시되어 재클릭 시 재요청 없음
- 항목 클릭(선택)과 쉐브론 클릭(펼침)이 독립 동작
