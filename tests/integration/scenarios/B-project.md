...
> **UI 구조 주의**: "새 프로젝트" 다이얼로그에는 **프로젝트 경로 입력란 1개와 BMad 체크박스**, 그리고 경로 입력란 라벨 행 우측에 **"찾아보기" 버튼**이 존재한다. 별도의 "이름" 필드가 없으며, 경로의 **말단 디렉토리명이 프로젝트 이름**으로 사용된다. 따라서 고유한 테스트 프로젝트 이름은 경로 말단에 포함시켜야 한다 (예: `C:\Users\<user>\AppData\Local\Temp\__hammoc_test_B-02-01_<timestamp>__`). **"찾아보기"는 디렉토리 트리 다이얼로그를 열어 경로를 채우는 *보조* 입력 경로일 뿐이며, 텍스트 입력이 여전히 주 경로다** — B-02-01/02 절차는 텍스트 입력을 그대로 사용하고(절차 불변), 디렉토리 브라우저 흐름은 B-02-03 에서 검증한다.
...
### B-02-03: 찾아보기로 디렉토리 트리 선택 후 생성 `[CORE] [MOBILE]`
**목적**: 경로를 타이핑하지 않고 "찾아보기" 디렉토리 트리에서 시각적으로 폴더를 선택해 프로젝트를 생성한다. 모바일에서 원격 PC 경로 타이핑을 회피하는 것이 핵심 가치. **SDK 비의존** — 순수 클라이언트 UI + `/api/system/browse*` REST 계약(Epic 34).

> **UI 주의**: "찾아보기" 버튼은 경로 입력란 라벨 행 우측에 있다(visible text `찾아보기`, `aria-label="디렉토리 찾아보기"`). 클릭 시 부모 다이얼로그(z-50) 위에 **z-[60] 중첩 다이얼로그**("디렉토리 선택" 헤더)가 열린다. 트리 행은 `role="treeitem"`, **폴더만** 노출되며 `hasChildren` 인 항목에만 펼침 chevron(`aria-expanded` 속성)이 붙는다. **삭제 액션은 어디에도 없다**(컨텍스트 메뉴/삭제 버튼 부재).

**절차**:
1. "새 프로젝트" 버튼 클릭 → 다이얼로그 오픈
2. 경로 입력란 옆 "찾아보기" 클릭 → "디렉토리 선택" 다이얼로그 오픈 확인 (`browser_snapshot`)
3. 다이얼로그가 **유저 홈을 펼친 상태로 시작**하는지 확인: 상단 브레드크럼에 "내 PC" + 홈 경로 세그먼트가 표시되고 트리에 홈 하위 폴더가 렌더됨. 네트워크 로그에 `GET /api/system/browse`(무인자 — home 획득) → `GET /api/system/browse?path=<home>`(트리 자식) **2-콜** 확인
4. 임의의 하위 폴더 행 클릭 → 펼침 시 `GET /api/system/browse?path=<해당 폴더 절대경로>` 호출되고 하위 폴더가 lazy-load 됨. **파일은 표시되지 않음**(폴더만) 확인
5. 프로젝트를 만들 폴더를 선택 → 하단 "이 경로 선택" 버튼 enabled + 하단 선택 경로 미리보기 갱신
6. (선택) 툴바 "새 폴더" 클릭 → 인라인 입력 노출 → 고유 테스트명(`__hammoc_test_B-02-03_<timestamp>__`) 입력 후 Enter → `POST /api/system/browse/mkdir` **201** → 새 폴더가 트리에 나타나면 선택
7. "이 경로 선택" 클릭 → 브라우저 다이얼로그가 닫히고, 새 프로젝트 다이얼로그의 **경로 입력란이 선택 경로로 채워짐** 확인 (`browser_evaluate` 로 input value 검증)
8. 경로 validation 완료(공통 타이밍 주의 — "생성" enabled) 대기 → "생성" 클릭 → `/project/<slug>` 자동 이동

**기대 결과**:
- 선택한 절대경로가 경로 입력란에 그대로 반영되고, **기존 `validatePath → createProject` 등록 흐름**으로 프로젝트가 생성됨 (찾아보기는 input 을 채우는 보조 경로일 뿐 — 등록 경로/검증 로직 불변)
- `/api/projects` 응답에 새 프로젝트 포함
- 디렉토리 브라우저는 경로 히스토리/즐겨찾기/파일 미리보기를 남기지 않음 (비범위 — `localStorage`/스토어 적재 없음)

**엣지케이스**:
- E1. 브레드크럼 "내 PC" 클릭 → 드라이브 루트 뷰로 전환 (`GET /api/system/browse` 무인자, `isDriveRoots:true`). Windows 는 드라이브 문자(`C:` 등), Unix 는 `/`. 드라이브 항목 클릭으로 진입 가능
- E2. 파일 미표시: 트리에는 폴더만 노출되며 일반 파일은 어떤 깊이에서도 나타나지 않음 (서버가 폴더만 반환)
- E3. `[MOBILE]` 좁은 뷰포트: 다이얼로그가 하단 시트(`items-end`)로 표시되고, 툴바 버튼 라벨은 아이콘만 남으며(`hidden sm:inline`) 탭 타깃 44px 유지. 브레드크럼은 가로 스크롤
- E4. 권한 없는 디렉토리 펼침: 해당 노드 옆 인라인 에러 + "재시도" 링크 (전역 에러 화면으로 승격되지 않음 — 다이얼로그 로컬 에러)
- E5. 중복 폴더명으로 "새 폴더"/"이름 바꾸기": `409 ALREADY_EXISTS` → 트리 하단 인라인 에러 표기, 다이얼로그는 닫히지 않음
