...
**기대 결과**:
- 응답이 **블록 단위로** 나타남 — SDK 모드의 토큰 단위 스트리밍과 달리 완성된 블록이 한 번에 렌더. `cliShowGenerationProgress` ON 이면 생성 중 진행률 인디케이터 노출 (OFF 면 미노출) — **CLI 스피너와 동일 포맷**(시간 먼저·토큰 나중): `{경과} · ↓ {토큰} tokens`, 시간 단위는 분/초(`45s`·`1m 4s`, mm:ss 콜론 아님), 토큰은 1000 단위 `k` 축약(`↓ 2.3k tokens`). 스피너는 **브레일 회전 글자**(점 밝기 변화가 아님), 경과 시간은 **클라이언트가 응답 시작 시점부터 직접 잰 실측**(claude 화면에서 긁은 초가 아님)
...
3. **생성 중** 화면 관찰 — 본문 텍스트·도구 카드가 *그 순간* 뜨는지, 그 카드들이 **흐리게** 표시되고 **각 카드 제목 옆**(도구명·"Claude"·생각 헤더)에 **`미리보기` 칩**이 붙는지, 도구 임시본의 입력이 비어 보이는지
4. **응답 완료 후** 같은 카드들이 흐림 없이 **정본으로 교체**되고 상태줄 `미리보기` 표시가 사라지는지(턴 종료 reload), 도구 입력이 채워지는지
5. (긴 답변) 화면을 넘기는 **여러 도구/사고 카드**를 쓰게 하는 작업 송신 → 위쪽 카드가 스크롤로 사라진 뒤에도 **새 카드가 라이브로 계속** 뜨는지

**기대 결과**:
- **단일 라이브 소스**: 본문 텍스트가 thinking·도구와 **같은 소스(화면)** 에서 즉시 임시본으로 표시 — 텍스트가 파일 드레인을 기다리느라 순서가 틀어지는 레이스 없음. 드레인은 정본 치환만 담당(텍스트 정본도 thinking·도구와 **같은 순서 큐(reveal)** 를 거쳐 제자리 치환 — 즉시 반영하면 타이핑 큐에 아직 든 미리보기를 못 찾아 **중복 카드**가 생기므로. thinking 은 원래 양쪽 다 reveal 경유라 정상이었고, 텍스트만 정본이 큐를 우회하던 비대칭을 제거)
- **임시본 시각 구분**: 임시본 카드(텍스트·thinking·도구·결과)는 **불투명도 ~0.65**(색 단독 아님 — 색각 비의존) + **각 카드 제목 옆에 `미리보기` 칩**(도구명·"Claude"·생각 헤더 옆 — 카드 오른쪽 끝이 아니라 제목 바로 옆). 검증 셀렉터는 색이 아닌 **카드의 `data-provisional` 속성 + `미리보기` 칩(`data-provisional-badge`)**으로 단언
- **도구 임시본 = 이름만**: 화면이 도구 입력을 truncate 해 도구 임시본은 `input` 이 빈 채 뜸(가장 뚜렷한 "확정 전") → 정본 reload 가 전체 입력을 채움
- **정본 치환**: 턴 종료 `stream:complete-messages` reload 후 **전 카드 흐림 해제 + `미리보기` 칩 사라짐 = 정본** — 이중 렌더 0(SDK 와 동일 메커니즘)
