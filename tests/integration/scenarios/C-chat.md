...
> **자동 회귀**: 누적 dedup(스크롤-아웃)·텍스트 임시본 emit·드레인 정본 치환(텍스트 정본도 미리보기와 동일 reveal 경로로 제자리 교체)·푸터/본문 비충돌·파일 백스톱·37.10 4불변식(권한 슬롯 예약·합성 id 전이·green-only flip·빈-정본 thinking 보존)·**깜빡임-복원 stickiness(글리프 빠진 도구 헤더를 직전-관측 기반으로 `●` 복원 → 융합 방지)**·**같은-내용 도구 색전이 분리(완료=초록 본 도구는 같은 내용 실행중=회색 카드와 매칭 안 하고 은퇴 → 2번째 인스턴스가 자기 카드+완료, Story 37.13)**·**스크롤오프 도구 파일-백스톱(Story 37.16 — 화면 green 못 본 채 뷰포트 위로 스크롤된 실행중 도구를 파일 `tool_result` 가 임시 id 로 완료 / 화면 flip 본 슬롯은 중복 스킵, 재현 2종: 스크롤오프 완료 + green 본 중복-0)**는 `cliChatEngine.test.ts`·`cliGridCards.test.ts` 가 결정적 그리드 입력으로 **직접** 커버; `provisional` 플래그 전파(텍스트/thinking/도구)와 차등 렌더(카드 `data-provisional` + 각 카드 제목 옆 `잠정` 칩[`ProvisionalBadge`])는 `chatStore.test.ts`·`MessageArea.test.tsx`·`ThinkingBlock.test.tsx`. thinking 진행중·이전-턴 잔재·거대 블록(Tier 2)은 실 PTY/구독 의존이라 수동(오너 라이브, quota 확보 후).
