...
- E4. **CLI auto-compact 후 턴 지속 (CLI 전용, E3 의 후속)**: E3 로 메시지가 전송돼 컨텍스트 한계에서 claude 가 일반 메시지 처리 *도중* 자동 압축하면, 세션 기록에 압축 경계(`compact_boundary`)가 남고 **그 뒤에 원래 메시지에 대한 실제 응답이 이어진다**(실측 2026-06-17: 경계 직후가 아니라 ~3분 후 응답 시작 + 이후 ~1000줄 후속 작업). CLI 엔진은 경계의 `trigger` 로 분기한다 — `manual`(사용자가 컨텍스트 링 클릭 = `/compact`)은 압축이 곧 턴의 끝이라 경계에서 턴 종료(기존 compact-hang 방지 유지), `auto`(턴 도중 자동 압축)는 경계를 **턴 종료로 보지 않고** '압축 중' 표시(`system:compact`)만 한 뒤 계속 추적해 뒤따르는 `end_turn` 으로 정상 종료한다. 이전엔 둘을 구분 못 해 경계를 보자마자 무조건 턴을 끝내, auto-compact 직후 이어질 응답이 유실됐다(증상: "자동 압축 후 바로 멈춤"). 단위 `cliChatEngine.test.ts` 의 auto→턴 미종료+후속 응답으로 완료 / manual→경계에서 완료 2종이 직접 커버(90% 도달 비용 무관, 자동화됨).
...
**엣지케이스**:
- E1. **새로고침 후 즉시 선표시(브라우저 로컬 캐시)**: 대화가 있는 세션에서 `location.reload()` → 서버 `stream:history` 도착을 기다리지 않고 직전 대화가 로딩 스켈레톤 없이 즉시 렌더되고, 곧 서버 정본으로 교체된다(stale-while-revalidate). 메시지 스토어가 정본을 받을 때마다 직전 transcript 를 브라우저 로컬에 스냅샷해 두기 때문. 검증:
  ```js
  // 새로고침 직후 — 직전 세션 transcript 가 로컬 스냅샷에 보존됨
  browser_evaluate(`() => { const c = JSON.parse(localStorage.getItem('hammoc-session-snapshot') || '{}'); return Object.keys(c.sessions || {}).length; }`)
  // ≥ 1
  ```
  새로고침 후에도 `message-bubble` 카운트가 유지(빈 화면 깜빡임 없음). 멀티 브라우저 안전: 스냅샷은 그 브라우저에게만 보이는 선표시일 뿐 서버가 단일 출처라, 다른 기기에서 진행된 내용은 도착한 `stream:history` 가 덮어쓴다.
...
> **범위 (Epic 33)**: 전역 설정에서 **CLI** 엔진을 선택한 세션에서, 대화가 SDK 모드와 *동일한 세션 JSONL* 로 렌더·복원되는지. SDK 모드와의 차이는 *블록 단위 렌더*(완성된 블록이 한 번에 렌더 — 토큰 단위 타이핑 애니메이션 없음. 실시간성은 잠정 카드(화면 스크레이프)가, 정확성은 정본 교체가 담당)와 생성 진행률 인디케이터(`{경과} · ↓ N tokens` — 시간 먼저·분/초·토큰 `k` 축약, 328 줄 상세).
...
**기대 결과**:
- 응답이 **블록 단위로** 나타남 — SDK 모드의 토큰 단위 스트리밍과 달리 완성된 블록이 한 번에 렌더. `cliShowGenerationProgress` ON 이면 생성 중 진행률 인디케이터 노출 (OFF 면 미노출) — **CLI 스피너와 동일 포맷**(시간 먼저·토큰 나중): `{경과} · ↓ {토큰} tokens`, 시간 단위는 분/초(`45s`·`1m 4s`, mm:ss 콜론 아님), 토큰은 1000 단위 `k` 축약(`↓ 2.3k tokens`). 스피너는 **브레일 회전 글자**(점 밝기 변화가 아님), 경과 시간은 **claude 스피너에서 긁은 값**(`generationProgress.elapsedSeconds`, 클라 자체 1초 타이머에서 전환 — 미러 화면과 정확히 일치). 단 토큰이 멈추면 시계도 멈추고 카운터만 바뀌는 프레임엔 0으로 튈 수 있음(파싱 일치를 위해 수용한 트레이드오프)
...
- 한 턴의 블록(텍스트+도구 카드)이 완성되는 대로 즉시 렌더 — 도구 카드는 화면 스크레이프로 잠정 표시됐다가 파일 정본으로 제자리 교체되며 완료. (타이핑·카드 지연 연출은 제거됨 — 잠정 카드가 실시간성을 담당하므로 불필요했고, 도구 완료가 타이핑 큐 뒤에 묶여 영영 끝나지 않던 부작용이 있었음.)
...
- E3. 토큰 카운터(`↓ N tokens`)가 멈추면 `Ns` 경과초도 같이 멈출 수 있음 — 시간도 같은 스피너 파싱 값(`elapsedSeconds`)이라(클라 타이머 아님), 매끄러운 증가 대신 화면 갱신에 맞춰 튀거나 0으로 읽힐 수 있음(미러와의 정확한 일치를 위해 수용)
...
- **선택지 설명**: 선택지 카드의 각 항목이 라벨만이 아니라 **화면에 그려진 항목별 설명문**을 라벨 아래 함께 표시 — claude TUI 가 각 옵션 라벨 아래 들여써 칠하는 설명 줄을 그리드에서 긁어 카드로 전달(예전엔 라벨만 긁고 설명 줄을 버려 카드가 단순했음). 단일선택도 설명을 본문으로 노출(이전엔 호버 툴팁만), 설명이 없는 옵션은 라벨만
...
- E6. 항목별 설명 캡처: 옵션 라벨 아래 들여쓴 설명 줄을 **다음 번호 옵션 전까지** 그 옵션 설명으로 귀속(여러 줄이면 공백으로 합침), 설명 줄이 없는 옵션은 `{label}` 그대로, 박스 chrome 줄·`Type something`/`Chat about this` 어포던스 줄은 설명에 섞이지 않음. 다중질문(탭) 모달도 같은 본문 스크랩을 재사용하므로 탭별 설명까지 동일하게 캡처

> **자동 회귀**: 사용-한도 감지(고갈 중지 + 97% 경고 통과 + **주입 전 재개 repaint 무시 + 실제 사용량 대조 오탐 차단**), 토큰 첫-프레임 융합 차단, 질문 선행 텍스트 dedup, **그리드 기반 권한/질문/한도 감지 + 박스 chrome 라벨 정합(32.8) + 항목별 설명 캡처** 는 `cliChatEngine.test.ts`(엔진 배선, 박스-렌더 프레임으로 재저작) 와 `cliModalDetect.test.ts`(순수 디텍터/파서를 그리드 행으로 직접 단정) · `cliModalDetect.realframes.test.ts`(실측 PTY 프레임에서 라벨+설명 동시 스크랩) · 카드 표시는 `InteractiveResponseCard.test.tsx`(단일선택 설명 본문 노출) 가, 사용량 파싱(5h·주간·Opus·Sonnet)·대조 가드는 `rateLimitProbeService.test.ts` 가, CLI 모드 무타임아웃은 `websocket.test.ts`(Story 35.1 블록) 가 커버. 수동 단계는 실제 PTY/구독/한도에서의 *체감*만 확인.
...
### C-11-08: CLI 미러 — 서버 완성 화면 색 포함 실시간 전송 + 접기/펴기 복원 (Story 37.8) (수동)
**배경**: 37.7 까지는 미러가 ① 매 raw PTY 프레임(제자리 덮어쓰기 *델타*)을 라이브 broadcast 하고, ② 늦은 합류/새로고침에만 `session:join` 시 평문 그리드 스냅샷을 1회 보냈다. 이 구조의 한계: (a) 델타는 빈 화면에 적용하면 복원 불가 — **미러 패널을 접었다 펴면**(같은 소켓·같은 세션이라 `session:join` 재발화 X) 새 빈 xterm 에 델타만 와서, 턴이 끝나 델타가 멈추면 **영영 빈 화면**, (b) 스냅샷은 평문 그리드라 **색이 빠짐**. 이제는 37.1 헤드리스 스크린 모델이 어차피 매 프레임 들고 있는 **현재 화면을 serialize 애드온으로 색(ANSI)까지 직렬화한 자기완결 프레임** 을 ~100ms trailing throttle 로 **통째 실시간 전송**(`cli:screen-frame`)한다. 매 프레임이 완성 화면이라 클라는 `reset()`+`write(frame)` 로 그리기만 하면 되고 — 라이브·late-join·새로고침·**접기/펴기** 가 모두 한 경로로 수렴한다. 델타(`cli:pty-raw`)+그리드 스냅샷(`cli:screen-snapshot`) 이원 구조는 이 단일 프레임으로 **일원화**. 접기/펴기·재마운트는 `session:join` 을 재발화하지 않으므로, 패널이 마운트 시 `cli:request-screen-frame`(제어 채널)으로 캐시된 현재 프레임을 1회 당겨온다. 캐시는 턴 중 throttle 과 함께 실시간 갱신 + 턴 종료 시 최종 프레임 승계. 미러는 **읽기 전용**.

**선행 조건**: C-11-01 과 동일(구독 로그인된 `claude` 바이너리). CLI 엔진 선택 상태. 같은 세션을 동시에 볼 둘 이상의 브라우저/기기(예: 데스크톱 + 폰).

**스텝**:
1. 전역 설정 → CLI 엔진 선택. (미러 설정을 건드리지 않은) 새 세션에 프롬프트 송신 → 미러 패널이 **기본으로 접힌 채**(제목 바만 보임) 시작하는지 확인. 제목 바를 한 번 탭/클릭해 **펴면** 생성 중 화면이 **색까지** 라이브로 비치는지 확인
2. 생성 중/후 미러 패널을 **접었다가 다시 편다** → 펴는 즉시 **현재 claude 화면(색 포함)** 으로 복원되는지 확인(빈 화면으로 남지 않음). 특히 **턴이 끝나 조용한 상태** 에서 접었다 펴도 복원되는지 확인(예전엔 영영 빈 화면)
3. 한 턴이 끝난 뒤(또는 진행 중) **새 브라우저 탭/기기** 로 같은 세션을 연다 → 미러는 (기본 접힘이라) 접힌 채 합류하고, **펴면** 즉시 현재 화면(색 포함)으로 채워지는지 확인
4. 미러를 펴 둔 브라우저에서 **새로고침(reload)** → 패널은 기본값인 접힌 상태로 돌아오고(접힘 상태는 비영속), **다시 펴면** 즉시 현재 화면으로 복원되는지 확인
5. 데스크톱과 폰을 나란히 두고 → 두 화면이 같은 claude 화면을 보이는지(서버 단일 출처) 확인. 미러 영역에 키를 쳐도 claude 에 입력이 들어가지 않음(읽기 전용)
6. (opt-out 회귀) "CLI Mode Settings" 에서 **claude 화면 미러** 토글을 끈다 → 미러 패널이 사라지고 `cli:screen-frame` 이 더는 오지 않는지 확인. 다시 켜면 복귀
7. (깜빡임 육안) 스피너가 도는 동안 미러를 응시 → 매 프레임 reset+write 로 인한 **깜빡임/스크롤 누적** 이 거슬리지 않는지 확인(거슬리면 더블버퍼링 후속 과제)
8. (가변 폭·고정 글자) 채팅 레이아웃을 **와이드 ↔ 표준** 으로 토글 → 펼친 미러 패널의 폭이 대화 컬럼과 함께 변하는지 확인(표준=중앙 정렬·최대 80rem, 와이드=전체 폭, 메시지 영역/입력창과 좌우 끝 일치). 이때 **글자 크기는 변하지 않아야** 하고, 120칸이 패널보다 넓으면(좁은 폭/모바일) **가로 스크롤**이 생겨 좌우로 밀어 보는지 확인(글자를 줄여 욱여넣지 않음)
9. (포커스·키보드) **데스크톱**: 펼친 미러 영역을 클릭 → 미러에 포커스 링이 생기지 않고 채팅 입력창 포커스/커서가 그대로 유지되는지 확인. **모바일/폰**: 미러를 펴거나 미러 영역을 터치 → 가상 키보드가 **뜨지 않는지** 확인(미러는 손가락으로 위아래·좌우 스크롤만 됨)
10. (리사이즈 하단 고정) 미러 제목 바를 위/아래로 드래그해 높이를 키웠다 줄였다 → claude 의 **입력 박스/스피너(화면 하단)** 가 계속 보이고, 높이 변화가 **위쪽**에서 일어나는지 확인(예전엔 상단이 고정돼 키우면 하단이 가려졌음)
11. (미러가 채팅 입력바를 밀어내지 않음) 미러를 크게(예: 최대 높이까지) 펼친 상태에서 화면 세로가 짧아지는 상황을 만든다 — 데스크톱은 창을 낮게 줄이고, **모바일/폰은 채팅 입력창을 탭해 가상 키보드를 띄운다** → 채팅 **하단 입력바(푸터)** 가 계속 **완전히** 보이는지 확인. 미러는 공간이 부족하면 **스스로 높이를 줄여(내부 세로 스크롤)** 입력바를 화면 밖으로 밀어내지 않음(예전엔 미러가 고정 높이를 고집해 짧은 화면/키보드에서 입력바가 화면 아래로 잘려 사라졌음)

**기대 결과**:
- **색 포함 실시간**: 생성 중 화면이 서버와 동일하게 **색까지** 라이브로 비침(델타+평문 스냅샷 이원 → 색 있는 완성 프레임 단일로 통일)
- **기본 접힘**: 새 CLI 세션은 미러가 **접힌 채** 시작(제목 바만) — xterm·소켓 구독은 첫 펼침에서 **지연 마운트**. 펼치면 현재 화면 복원
- **접기/펴기 복원**: 패널을 접었다 펴면(턴 종료 후 포함) 즉시 현재 화면으로 복원(예전엔 빈 화면 잔존 — 이번 수정의 핵심 증상)
- **late-join/새로고침 동기화**: 새 브라우저/기기/새로고침은 (기본 접힘이라) 접힌 채 합류하고, **펴면** 빈 화면이 아니라 현재 화면(색 포함)으로 1회 초기화(펼침 시 `cli:request-screen-frame` 로 서버 캐시 프레임 pull). 나아가 **같은 브라우저에서의 새로고침/재접속**은 펴는 즉시 직전에 본 프레임을 **브라우저 로컬 스냅샷으로 선표시**(서버 `cli:request-screen-frame` 왕복을 기다리지 않음)하고, 서버 프레임이 도착하면 같은 `reset()`+`write` 경로로 덮어쓴다(stale-while-revalidate) — CLI 미러는 한 프레임이 곧 전체 화면이라 가장 무거운 페이로드라서 이 선표시로 sleep-wake/재접속의 빈 화면 공백이 사라진다
- **멀티 브라우저 일관성·읽기 전용**: N 개 클라가 같은 화면(서버 단일 출처). 미러에 입력해도 PTY 에 닿지 않음
- **가변 폭·고정 글자**: 패널 폭은 와이드/표준 레이아웃 모드를 그대로 따라감(`content-container`) — 표준=최대 80rem 중앙 정렬, 와이드=전체 폭, 메시지 영역/입력창과 좌우 정렬 일치. **글자 크기는 폭과 무관하게 고정(13px)** 이고, 120칸이 패널보다 넓으면 가로 스크롤이 생김(이전엔 폭에 맞춰 글자를 6px까지 줄여 좁은 폭에서 안 읽혔음). 표준(narrow) 레이아웃에선 **접힌 제목 바도 컬럼 폭(80rem)을 꽉 채워 제목이 좌측 정렬** — 내용 너비로 쪼그라들어 가운데로 뜨지 않음(미러를 세로 shrink 가능한 flex 컬럼으로 바꾸면서 `content-container` 가 flex 아이템이 됐는데, 표준 레이아웃의 `margin-inline:auto` 와 만나면 stretch 가 풀려 내용 너비로 줄고 그 auto 마진이 가운데로 미는 함정 → `w-full` 로 컬럼 폭에 고정)
- **포커스 차단·키보드 억제**: 읽기 전용 미러는 **포커스를 가져가지 않음**(xterm 의 숨은 입력 textarea 를 `pointer-events:none`+`readonly`+`inputmode=none`+`tabindex=-1` 로 무력화) → 데스크톱 포커스 링 0, 모바일 가상 키보드 0. 미러 영역 터치는 스크롤 컨테이너로 흘러 위아래 스크롤만 동작
- **리사이즈 하단 고정**: 제목 바 드래그로 높이를 바꾸면 **하단(claude 활성 영역)이 고정**되고 위쪽이 가변(화면 높이는 고정이라 높이 변화량 Δ 만큼 scrollTop 을 −Δ 보정). 예전엔 상단 고정이라 키우면 하단이 가려졌음
- **미러가 채팅 입력바를 밀어내지 않음(하단바 항상 보임)**: 미러를 아무리 크게 펼쳐도 채팅 **하단 입력바(푸터)** 는 항상 완전히 보임. 미러는 `shrink-0`(고정) 대신 **유연한 flex 컬럼**(`flex flex-col min-h-0`)이라, 화면 세로가 짧거나(작은 화면) **모바일 키보드로 레이아웃 뷰포트(`dvh`)가 줄면** 설정된 높이를 고집하지 않고 **스스로 줄어들어(내부 스크롤)** 푸터 공간을 양보. 앱 전체가 `interactive-widget=resizes-content`(키보드 시 `dvh` 축소) 전제로 동작하는데 미러만 강성이라 푸터를 `overflow-hidden` 밖으로 밀어내던 회귀가 사라짐. 양보 우선순위 = 헤더·푸터(고정) > 미러(양보, 내부 스크롤) > 대화 영역(먼저 축소). 실측(414×430, Chromium): 미러 640→218px 자동 축소, 푸터 화면 하단에 고정(수정 전엔 푸터가 뷰포트 아래로 422px 잘림)
- **opt-out 존중**: 미러 토글 OFF 시 `cli:screen-frame` 0(서버 게이트 false). SDK 모드는 PTY/미러 자체가 없어 무관
- **격리**: 추출 경로(진행률 37.2/37.3·모달 37.4·주입 전 분류 37.6)는 평문 `readGrid` 를 그대로 쓰므로 불변(serialize 는 별도 경로, 추가 flush 없음)·턴당 에뮬레이터 dispose·37.5 권한 폐루프 불변

**엣지케이스**:
- E1. **캐시 미스 폴백**: 첫 턴이 끝나기 전(승계할 프레임 없음) 합류/요청하면 전송 생략 → 빈 화면 폴백(기존과 동일), 라이브 프레임이 도착하면서 채워짐 — 단 이 브라우저가 직전에 같은 세션을 본 적이 있으면 **브라우저 로컬 스냅샷**으로 선표시되어 서버 캐시 미스에도 빈 화면이 아니다(로컬 스냅샷마저 없을 때만 빈 화면)
- E2. **동일 프레임 skip**: 스피너가 멈춰 화면이 안 바뀌면 같은 프레임을 다시 안 보냄(대역폭 0) — 정지 화면은 마지막 프레임 그대로 유지
- E3. **세션 캐시 누수 0**: 세션 룸의 마지막 소켓이 떠나면 화면 캐시도 함께 정리(버퍼 파기와 동일 시점)
- E4. **normal buffer + alt 흡수**: 실측상 claude TUI 는 normal buffer 를 쓴다(alt 진입 `?1049h` 없음) — 출력이 위로 흐른다(→ E6). 혹 모달 등에서 일시적 alt 를 쓰면 직렬 프레임의 alt 진입+홈 시퀀스를 클라가 매 프레임 `reset()` 으로 흡수해 상태 누적 없음
- E5. **미러 기본 글자색 중립화**: claude 는 입력 프롬프트·평문에 색을 안 줘서 테마 기본 전경색으로 렌더된다. 공유 Tokyo Night 전경색(`#c0caf5`, 연보라/청록빛)을 **미러 한정** 중립 회색(dark `#d4d4d4` / light `#2e2e2e`)으로 override — claude 가 지정한 ANSI 색(16색 팔레트)은 불변이라 실제 터미널의 무채색 텍스트와 비슷해진다
...
> **자동 회귀**: serialize 색 보존 + readGrid 평문 유지는 `cliScreenModel.test.ts`, trailing throttle(leading 즉시·burst 최신 1회·cancel)은 `trailingThrottle.test.ts`, 세션 화면 캐시(프레임 문자열) set/get/delete 는 `cliScreenCache.test.ts`, 게이트(`shouldForwardCliPtyMirror`: CLI+undefined→true·false→false·SDK→false)는 `cliEngineUtils.test.ts`, `session:join` 캐시 히트 시 `cli:screen-frame` 1회·미스 0회·같은 소켓 재-join dedup 0회 는 `websocket.test.ts`, **기본 접힘(마운트 시 구독·`new Terminal()` 0·복사 버튼 부재)→첫 펼침에서 지연 마운트(구독+`cli:request-screen-frame` emit)**·프레임 수신 시 `reset()`+`write(frame)`·매 프레임 재-reset·cleanup off·**브라우저 로컬 캐시 선표시(마운트 시 캐시 프레임을 먼저 그림 + 들어온 프레임을 현재 세션별로 캐시 저장)** 는 `CliPtyMirror.test.tsx` 가, **세션 스냅샷 캐시(메시지/프레임 set·load·pending merge·LRU 5세션·크기 가드·tail 컷·손상 복구)** 는 `sessionSnapshotCache.test.ts`·`messageStore.test.ts`(정본 수신 시 현재 세션 transcript 캐시) 가 커버. 수동 단계는 실제 PTY/구독·다중 기기에서의 *육안* 화면 동기화·색 보존·접기펴기 복원·깜빡임 + **가변 폭(와이드/표준)·포커스 차단·모바일 키보드 억제·리사이즈 하단 고정·미러가 채팅 입력바를 밀어내지 않음(짧은 화면/키보드 시 하단바 유지)**(모두 실제 xterm DOM/뷰포트 의존이라 jsdom 단위로는 검증 불가 — 마지막 항목은 실제 flex 레이아웃이 필요해 단위 불가, 별도로 Chromium 414×430 에서 푸터 가시성 실측)만 확인(PTY 화면은 dev shell 재현 불가).
...
- **주관식(Other) 자유 입력 (Story 37.15)**: 단일선택 질문에서 답이 옵션에 없으면(custom/Other) **"Type something" 항목으로 드라이브** — claude TUI 모달은 실옵션 다음에 `3. Type something`(자유입력)·`4. Chat about this`를 둔다(실측 probe-askq, claude v2.1.177). 시퀀스: ↓×옵션수로 "Type something" 이동 → Enter(입력 모드 진입) → 텍스트 타이핑 → Enter(제출). 모달에서 *직접* 타이핑은 무시되고 Enter는 하이라이트 옵션을 고르므로, 반드시 "Type something"을 먼저 골라야 한다. multiSelect·다중질문(탭) custom은 아직 미지원
- **안전 취소**: 탭이 안 넘어가거나(정지)·multiSelect/다중질문 답이 옵션에 없거나·파싱 실패면 Esc 로 취소 — 잘못된 선택 대신 깨끗한 종료(최악=기존 동작)
...
### C-11-10: CLI 스트리밍 단일 소스 + 임시본/정본 시각 구분 (Story 37.11) (수동)
**배경**: 37.9/37.10 은 thinking·도구·결과 카드를 **화면 그리드**에서 라이브로 긁어 띄웠지만 **텍스트는 파일(JSONL 드레인)** 에서 와, 같은 턴 카드가 두 비동기 소스(화면/파일)에서 나와 순서가 흔들리는 **혼합-소스 레이스**가 있었다. 37.11 은 텍스트까지 화면 그리드의 임시본으로 통일해 **모든 라이브 내용을 한 소스**에서 emit한다(드레인은 라이브 emit 중단→정본 치환 전용). 또한 ① 카드 dedup 을 *뷰포트 위치 인덱스*에서 *누적 보관(held transcript)* 기반으로 바꿔, 긴 답변에서 **옛 카드가 화면 밖으로 스크롤돼도 그 아래 새 카드를 놓치지 않게** 했고, ② 화면 파싱 **임시본**(아직 확정 전·lossy)과 파일 파싱 **정본**을 카드에서 **시각적으로 구분**한다(임시본 = 불투명도 ~0.65 + `live` *텍스트* 배지[색각 비의존, 색 단독 아님], 턴 종료 reload 시 정본으로 치환되며 표식 해제). 특히 **도구 임시본은 화면이 입력을 truncate 해 `input` 이 빈 채** 떠 가장 뚜렷하게 "확정 전" 임을 보인다.
   > **Tier 경계(중요)**: 이 스토리는 quota 불요 **Tier 1**(텍스트 단일 소스화 + 누적 dedup + 임시본/정본 시각 구분)만 구현·Done 했다. **Tier 2**(① thinking *진행중* `Thinking…` 즉시 포착으로 ~7초 갭 해소 ② resume 재칠 이전-턴 잔재 완전 차단 ③ 단일 초장문 블록 scrollback)는 오너 subscription quota 로 실 PTY 프레임을 캡처해야 착수 가능해 **게이트 WAIVE 로 deferred**(가정 강행 금지). 본 시나리오는 Tier 1 만 검증한다.

**선행 조건**: C-11-01 과 동일(구독 로그인된 `claude` 바이너리). CLI 엔진 선택 상태.

**스텝**:
1. 전역 설정 → CLI 엔진 선택, 권한 모드 **Bypass**(도구가 막히지 않게)
2. 텍스트 + 도구를 함께 쓰게 하는 메시지 송신(예: `이 저장소 구조를 한 문단으로 설명하고, package.json 의 name 을 Read 로 읽어서 알려줘`)
3. **생성 중** 화면 관찰 — 본문 텍스트·도구 카드가 *그 순간* 뜨는지, 그 카드들이 **흐리게** 표시되고 **각 카드 제목 옆**(도구명·"Claude"·생각 헤더)에 **`잠정` 칩**이 붙는지, 도구 임시본의 입력이 비어 보이는지
4. **응답 완료 후** 같은 카드들이 흐림 없이 **정본으로 교체**되고 상태줄 `잠정` 표시가 사라지는지(턴 종료 reload), 도구 입력이 채워지는지
5. (긴 답변) 화면을 넘기는 **여러 도구/사고 카드**를 쓰게 하는 작업 송신 → 위쪽 카드가 스크롤로 사라진 뒤에도 **새 카드가 라이브로 계속** 뜨는지

**기대 결과**:
- **단일 라이브 소스**: 본문 텍스트가 thinking·도구와 **같은 소스(화면)** 에서 즉시 임시본으로 표시 — 텍스트가 파일 드레인을 기다리느라 순서가 틀어지는 레이스 없음. 드레인은 정본 치환만 담당(텍스트 정본도 thinking·도구와 **동일하게 reveal 경로**로 제자리 치환 — 버퍼 텍스트를 먼저 flush 한 뒤 적용해 잠정 카드를 제자리에서 찾아 교체)
- **임시본 시각 구분**: 임시본 카드(텍스트·thinking·도구·결과)는 **불투명도 ~0.65**(색 단독 아님 — 색각 비의존) + **각 카드 제목 옆에 `잠정` 칩**(도구명·"Claude"·생각 헤더 옆 — 카드 오른쪽 끝이 아니라 제목 바로 옆). 검증 셀렉터는 색이 아닌 **카드의 `data-provisional` 속성 + `잠정` 칩(`data-provisional-badge`)**으로 단언
- **도구 임시본 = 이름만**: 화면이 도구 입력을 truncate 해 도구 임시본은 `input` 이 빈 채 뜸(가장 뚜렷한 "확정 전") → 정본 reload 가 전체 입력을 채움
- **정본 치환**: 턴 종료 `stream:complete-messages` reload 후 **전 카드 흐림 해제 + `잠정` 칩 사라짐 = 정본** — 이중 렌더 0(SDK 와 동일 메커니즘)
- **누적 dedup(스크롤-아웃)**: 긴 답변에서 옛 카드가 뷰포트 밖으로 밀려나도 그 아래 새 카드가 라이브로 잡힘(위치-인덱스 대신 누적 보관 기반)
- **깜빡임 복원(Story 37.12)**: 실행 중 도구 헤더의 `●` 불릿은 깜빡인다(claude 가 다시 그림) — 60ms 폴링이 글리프 빠진 한 프레임을 잡아도, 직전 프레임에서 도구로 본 줄은 `●` 를 되살려 **도구가 본문 텍스트로 융합되지 않음**(`Search(…)` 헤더가 답변 본문에 합쳐지던 증상 방지). 복원은 *실제로 도구로 관측된 내용*에만 적용 → 괄호 든 본문(`foo(x)`) 오인 0, 스크롤로 사라진 도구는 잔상 0
- **리페인트 echo 중복 차단(Story 37.18)**: claude 가 실행 중(회색) 도구를 다시 그리며 같은 회색 도구 줄을 스크롤백에 **2줄** 남기면(실측 ba310cea 덤프: 같은 `● Bash(git diff)` 가 row 231·242 둘 다 회색, 252 초록), 37.13 의 '같은 내용 = 별 인스턴스' 규칙이 이를 **2 카드로 중복**시킨다. 새 카드 생성 직전 **echo 가드** 로 차단 — 같은 프레임에 이미 잡힌 *미완료(non-green)* 같은 내용 도구가 있으면 redraw 흔적으로 보고 건너뜀(진짜 2번 *호출* 은 green→gray 색 전이라 `supersededDone` 으로 구분돼 새 카드 유지). 이 중복은 **정본 교체 실패**(2번째 증상)도 유발했다: 화면 2카드 vs 파일 `tool_use` 1개 → maybeFinalize per-kind FIFO 가 1개만 교체하고 1개는 미리보기 badge 로 잔존(`provisionalToolEmitsPending` 누수) → echo 차단으로 화면=파일 1:1 되어 정상 교체
- **같은 종류 도구 반복 완료 감지(Story 37.13)**: 같은 내용의 도구가 연속 실행될 때(특히 앞 도구가 위로 스크롤돼 화면에서 사라진 뒤) 뒤 도구가 앞 도구로 합쳐지지 않고 **각자 카드로 뜨고 각자 완료(초록 불릿)가 잡힌다**. 불릿 색은 회색(실행)→초록(완료) 한 방향이므로, **이미 완료(초록)로 본 도구**에 같은 내용의 **실행 중(회색)** 카드가 새로 오면 새 인스턴스로 판정(색 역행 = 새 도구) → 완료된 도구는 은퇴해 뒤 인스턴스를 흡수하지 않음(내용·순서 식별의 빈틈 보완). 이전 증상: 뒤 도구가 spinner 로 영영 남음
- **도구/텍스트 색 판별(Story 37.13)**: 같은 `●` 글리프라도 **불릿 색**으로 도구/텍스트를 가른다 — 초록(완료)·회색(실행)=도구, **흰색=어시스턴트 텍스트**(실측 cli-real-pty-dump 직접 검증: `● Update(...)`=green / `● 결론부터:…`=white). 이름 패턴(`Tool(`·하이픈 포함)은 *색 정보가 없을 때만* 폴백. → **하이픈 서브에이전트명**(`claude-code-guide`)도 도구로 인식되고, 본문에 우연히 `foo(bar)`가 있어도 흰색이라 텍스트로 분류 — 이름 패턴만으로 생기던 **양방향 오분류**(도구→텍스트 / 텍스트→도구)가 해소됨
- **완료도 색으로(Story 37.13)**: 도구 **완료**는 `⎿` 결과 줄이 *떠야가 아니라* **불릿이 green이면** 즉시 완료(spinner→체크). `⎿` 줄은 완료 *트리거*가 아니라 **output 소스**로만 — 같은 프레임에 있으면 결과 텍스트를 채우고, 긴 턴에서 화면 위로 스크롤돼 사라져도 green만 보면 완료(output은 턴종료 reload가 정본으로 채움). 이전 증상 ②(미러는 green인데 Hammoc 도구가 spinner로 영영 남음) 해소. `⎿`를 완료 구분자로 쓰던 fragile 의존 제거
- **스크롤오프 완료 백스톱(Story 37.16)**: green 불릿 *자체를 화면에서 못 보는* 경우 — 긴 답변에 **실행 중(회색)** 도구가 뷰포트 위로 스크롤돼 green 프레임이 화면 밖에서 일어남(화면 스크랩은 "아는 블록이 보이면 스크롤업 중단"이라 그 도구를 다시 못 읽음) — 에는 **파일 `tool_result` 가 백스톱**이 된다. 정본 확정 시점에 잡아 둔 진짜id↔임시 슬롯 매핑으로, 파일 결과가 오면 **화면 flip 과 동일 경로(임시 id + flip 플래그)** 로 완료시킨다(화면에서 이미 green 을 본 슬롯이면 중복은 스킵 — `gridResultFlippedSlots`). 클라는 화면 완료든 파일 백스톱이든 동일 신호를 받아 카드 분열 0. 이전 증상(긴 대화에서 도구가 턴 끝 reload 까지 spinner + 잠정 칩 안 풀림) 해소
- **들여쓴 출력 머리기호 차단(Story 37.17)**: 카드 헤더 글리프(`●`/`⎿`/`∴`/`Thought`)는 **좌여백 ≤ 3 컬럼**일 때만 카드를 연다(실측 cli-pty-dump 재생: `●` col 0·`⎿` col 2·`∴` col 0, 도구 출력은 col 5–7). bash 등 verbose 도구가 하위로 뱉는 출력(파일 미리보기·머리기호로 시작하는 출력 줄)이 trim 후 `●`/`⎿`로 시작해도 **카드 오검출 0** — 직전 카드의 continuation 으로 흡수. 파서(`parseGridCards`)뿐 아니라 flicker 복원/기억 헬퍼(`restoreFlickeredToolBullets`·`collectToolHeaderKeys`)도 동일 게이트(복원이 들여쓰기를 버리고 col 0 으로 만들어 게이트를 우회하던 경로 차단)
- **격리**: SDK 모드 회귀 0(SDK 는 `provisional` 미설정 → 항상 정본 렌더), 37.9 선행 텍스트 순서·37.10 thinking/도구 카드·미러(37.8) 회귀 0

**엣지케이스**:
- E1. (Tier 2 — 미구현) thinking 카드는 **완료 요약**(`Thought for Ns`)만 즉시 뜨고 *진행중* 표식은 아직 ~7초 갭이 남을 수 있음 — quota 확보 후 후속(본 시나리오 비대상)
- E2. (Tier 2 — 미구현) resume 직후 이전 턴 카드가 잠깐 임시본으로 비칠 수 있음(정본 reload 가 정정) — 완전 차단은 후속
- E3. 단일 블록이 뷰포트보다 길면 임시본이 잠깐 부분일 수 있으나 **시각 구분으로 "확정 전" 이 보이고** 파일 정본 reload 가 완전성 백스톱 → 데이터 손실 0
- E4. **텍스트 블록 사이 도구 끼임(분할 방지)**: 한 본문 텍스트 블록(같은 서버 messageId)이 그려지는 도중 도구/사고 카드가 끼어들어도 그 텍스트는 **하나의 잠정 세그먼트로 유지**(직전이 도구여도 같은 messageId 면 기존 세그먼트에 이어붙임) → 잠정 텍스트 개수와 정본 개수가 1:1 로 맞아 FIFO 정본 치환이 슬롯을 밀지 않음. 분할이 한 칸씩 누적되면 정본이 한 칸 일찍 떨어져 **정본 카드 아래에 같은 내용의 잠정 카드가 고아로 남던** 회귀의 직접 차단. 만약 분할이 생겨도 정본 치환이 **같은 messageId 형제 잠정 텍스트를 함께 흡수**(이중 방어)

> **자동 회귀**: 누적 dedup(스크롤-아웃)·텍스트 임시본 emit·**텍스트 블록 분할 방지(도구 끼임 시 같은 messageId 이어붙이기)+정본 치환 시 형제 흡수(고아 잠정 0, E4)**·드레인 정본 치환(텍스트 정본도 미리보기와 동일 reveal 경로로 제자리 교체)·푸터/본문 비충돌·파일 백스톱·37.10 4불변식(권한 슬롯 예약·합성 id 전이·green-only flip·빈-정본 thinking 보존)·**깜빡임-복원 stickiness(글리프 빠진 도구 헤더를 직전-관측 기반으로 `●` 복원 → 융합 방지)**·**같은-내용 도구 색전이 분리(완료=초록 본 도구는 같은 내용 실행중=회색 카드와 매칭 안 하고 은퇴 → 2번째 인스턴스가 자기 카드+완료, Story 37.13)**·**스크롤오프 도구 파일-백스톱(Story 37.16 — 화면 green 못 본 채 뷰포트 위로 스크롤된 실행중 도구를 파일 `tool_result` 가 임시 id 로 완료 / 화면 flip 본 슬롯은 중복 스킵, 재현 2종: 스크롤오프 완료 + green 본 중복-0)**는 `cliChatEngine.test.ts`·`cliGridCards.test.ts` 가 결정적 그리드 입력으로 **직접** 커버; `provisional` 플래그 전파(텍스트/thinking/도구)와 차등 렌더(카드 `data-provisional` + 각 카드 제목 옆 `잠정` 칩[`ProvisionalBadge`])는 `chatStore.test.ts`·`MessageArea.test.tsx`·`ThinkingBlock.test.tsx`. thinking 진행중·이전-턴 잔재·거대 블록(Tier 2)은 실 PTY/구독 의존이라 수동(오너 라이브, quota 확보 후).
