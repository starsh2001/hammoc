...
**기대 결과**:
- **단일 라이브 소스**: 본문 텍스트가 thinking·도구와 **같은 소스(화면)** 에서 즉시 임시본으로 표시 — 텍스트가 파일 드레인을 기다리느라 순서가 틀어지는 레이스 없음. 드레인은 정본 치환만 담당(텍스트 정본도 thinking·도구와 **같은 순서 큐(reveal)** 를 거쳐 제자리 치환 — 즉시 반영하면 타이핑 큐에 아직 든 잠정 카드를 못 찾아 **중복 카드**가 생기므로. thinking 은 원래 양쪽 다 reveal 경유라 정상이었고, 텍스트만 정본이 큐를 우회하던 비대칭을 제거)
...
> **자동 회귀**: 누적 dedup(스크롤-아웃)·텍스트 임시본 emit·드레인 정본 치환(억제 — **텍스트 정본을 미리보기와 동일 순서 큐로 태워 타이핑 큐 레이스 중복 0**, `provisional→canonical text replace race` 재현 케이스)·푸터/본문 비충돌·파일 백스톱·37.10 4불변식(권한 슬롯 예약·합성 id 전이·green-only flip·빈-정본 thinking 보존)·**깜빡임-복원 stickiness(글리프 빠진 도구 헤더를 직전-관측 기반으로 `●` 복원 → 융합 방지)**·**같은-내용 도구 색전이 분리(완료=초록 본 도구는 같은 내용 실행중=회색 카드와 매칭 안 하고 은퇴 → 2번째 인스턴스가 자기 카드+완료, Story 37.13)**·**스크롤오프 도구 파일-백스톱(Story 37.16 — 화면 green 못 본 채 뷰포트 위로 스크롤된 실행중 도구를 파일 `tool_result` 가 임시 id 로 완료 / 화면 flip 본 슬롯은 중복 스킵, 재현 2종: 스크롤오프 완료 + green 본 중복-0)**는 `cliChatEngine.test.ts`·`cliGridCards.test.ts` 가 결정적 그리드 입력으로 **직접** 커버; `provisional` 플래그 전파(텍스트/thinking/도구)와 차등 렌더(카드 `data-provisional` + 각 카드 제목 옆 `잠정` 칩[`ProvisionalBadge`])는 `chatStore.test.ts`·`MessageArea.test.tsx`·`ThinkingBlock.test.tsx`·`presentationQueue.test.ts`. thinking 진행중·이전-턴 잔재·거대 블록(Tier 2)은 실 PTY/구독 의존이라 수동(오너 라이브, quota 확보 후).
