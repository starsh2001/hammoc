...
3. **생성 중** 화면 관찰 — 본문 텍스트·도구 카드가 *그 순간* 뜨는지, 그 카드들이 **흐리게 + `live` 배지** 로 표시되는지, 도구 임시본의 입력이 비어 보이는지
4. **응답 완료 후** 같은 카드들이 흐림/배지 없이 **정본으로 교체**되는지(턴 종료 reload), 도구 입력이 채워지는지
5. (긴 답변) 화면을 넘기는 **여러 도구/사고 카드**를 쓰게 하는 작업 송신 → 위쪽 카드가 스크롤로 사라진 뒤에도 **새 카드가 라이브로 계속** 뜨는지

**기대 결과**:
- **단일 라이브 소스**: 본문 텍스트가 thinking·도구와 **같은 소스(화면)** 에서 즉시 임시본으로 표시 — 텍스트가 파일 드레인을 기다리느라 순서가 틀어지는 레이스 없음. 드레인은 정본 치환만 담당
- **임시본 시각 구분**: 임시본 카드(텍스트·thinking·도구·결과)는 **불투명도 ~0.65 + `live` 텍스트 배지**(색 단독 아님 — 색각 비의존). 검증 셀렉터는 색이 아닌 **`live` 배지 텍스트 / `data-provisional` 속성**으로 단언
- **도구 임시본 = 이름만**: 화면이 도구 입력을 truncate 해 도구 임시본은 `input` 이 빈 채 뜸(가장 뚜렷한 "확정 전") → 정본 reload 가 전체 입력을 채움
- **정본 치환**: 턴 종료 `stream:complete-messages` reload 후 **전 카드 흐림/배지 해제 = 정본** — 이중 렌더 0(SDK 와 동일 메커니즘)
...
> **자동 회귀**: 누적 dedup(스크롤-아웃)·텍스트 임시본 emit·드레인 정본 치환(억제)·푸터/본문 비충돌·파일 백스톱·37.10 4불변식(권한 슬롯 예약·합성 id 전이·green-only flip·빈-정본 thinking 보존)은 `cliChatEngine.test.ts` 가 결정적 그리드 입력으로 **직접** 커버; `provisional` 플래그 전파(텍스트/thinking/도구)와 차등 렌더(`data-provisional` + `live` 배지)는 `chatStore.test.ts`·`MessageArea.test.tsx`·`presentationQueue.test.ts`. thinking 진행중·이전-턴 잔재·거대 블록(Tier 2)은 실 PTY/구독 의존이라 수동(오너 라이브, quota 확보 후).
