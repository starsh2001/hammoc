...
**스텝**:
1. 전역 설정 → CLI 엔진 선택. (미러 설정을 건드리지 않은) 새 세션에 프롬프트 송신 → 미러 패널이 **기본으로 보이고** 생성 중 화면이 **색까지** 라이브로 비치는지 확인
...
3. 한 턴이 끝난 뒤(또는 진행 중) **새 브라우저 탭/기기** 로 같은 세션을 연다 → 합류 즉시 미러가 현재 화면(색 포함)으로 채워지는지 확인
4. 미러를 보고 있는 브라우저에서 **새로고침(reload)** → 직후 미러가 현재 화면으로 즉시 복원되는지 확인
...
- **late-join/새로고침 동기화**: 새 브라우저/기기/새로고침이 합류하면 빈 화면이 아니라 현재 화면(색 포함)으로 1회 초기화
...
**엣지케이스**:
- E1. **캐시 미스 폴백**: 첫 턴이 끝나기 전(승계할 프레임 없음) 합류/요청하면 전송 생략 → 빈 화면 폴백(기존과 동일), 라이브 프레임이 도착하면서 채워짐
