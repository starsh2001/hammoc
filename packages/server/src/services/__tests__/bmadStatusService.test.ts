...
describe('bmadStatusService', () => {
  ...
  // TC-BS-13c: 스토리 파일이 gate 보다 더 늦게 수정돼도 (예: review-story 가 gate 를
  // 먼저 쓰고 스토리의 QA Results 를 나중에 append) gateResult 는 정상 인식된다.
  // 과거엔 이 mtime 역전을 'Dev 가 fix 함' 으로 오판해 추천이 어긋났으나, 이제 mtime
  // 추측을 하지 않고 gateResult 만 보고한다 (회귀 방지).
  it('still reports gateResult when story file mtime > gate file mtime (no staleness inference)', async () => {
    (yaml.load as ReturnType<typeof vi.fn>).mockImplementation((content: string) => {
      if (typeof content === 'string' && content.includes('gate:')) {
