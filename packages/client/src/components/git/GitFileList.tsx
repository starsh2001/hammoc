...
function getStatusIndicator(type: 'staged' | 'unstaged' | 'untracked', file: GitFileStatus | string): { label: string; color: string } {
  ...
  switch (indicator) {
    ...
    default:
      return { label: indicator || '?', color: 'text-gray-500 dark:text-gray-400' };
  }
}
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 text-left"
      >
        {collapsed ? (
        ...
        <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
          {files.length}
        </span>
        {/* Group action button */}
        {staged && onUnstageAll && (
        ...
        <ul className="divide-y divide-gray-100 dark:divide-gray-800">
          {files.map((file) => {
            ...
            return (
              <li
                key={path}
                className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800/30 group"
              >
                {/* Status indicator */}
                ...
                <button
                  ...
                  className="flex-1 text-left text-sm font-mono text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 truncate"
                  disabled={isLoading}
                >
                  {path}
                </button>
