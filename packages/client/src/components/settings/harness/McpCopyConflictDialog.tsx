/**
 * Story 28.3: MCP server copy conflict dialog.
 *
 * 3-way resolution modal — overwrite / skip / rename — used both for the
 * initial copy attempt and for retries after the rename target itself
 * collides. Reserved-character validation runs inline and disables the submit
 * button until the name is OS-safe. Mirrors SkillCopyConflictDialog in scope
 * + behavior — only the i18n keys differ.
 */
...
export function McpCopyConflictDialog({
  defaultName,
  targetScope,
  errorMessage,
  onSubmit,
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="mcp-copy-conflict-title"
      ...
      >
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 dark:border-[#3a4d5e]/50">
          <h2
            id="mcp-copy-conflict-title"
            className="text-base font-semibold text-gray-900 dark:text-gray-100"
          >
            {t('harness.mcp.copy.conflict.title')}
          </h2>
          <button
            type="button"
            aria-label={t('harness.mcp.copy.conflict.close', { defaultValue: 'Close' })}
            onClick={onClose}
        ...
        <div className="px-5 py-4 space-y-3 text-sm text-gray-700 dark:text-gray-200 overflow-y-auto">
          <p>
            {t('harness.mcp.copy.conflict.intro', {
              targetScope: t(`harness.mcp.scopeBadge.${targetScope}`),
              name: defaultName,
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span>{t('harness.mcp.copy.conflict.overwrite')}</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
            ...
            />
            <span>{t('harness.mcp.copy.conflict.skip')}</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
            ...
            />
            <span>{t('harness.mcp.copy.conflict.rename')}</span>
          </label>

          {resolution === 'rename' && (
            <div className="ml-6 mt-1">
              <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                {t('harness.mcp.copy.conflict.renameLabel')}
              </label>
              <input
                type="text"
                value={renameInput}
              ...
              {isInvalid && (
                <p className="mt-1 text-xs text-red-600 dark:text-red-400">
                  {t('harness.mcp.copy.conflict.renameInvalid')}
                </p>
              )}
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-gray-100 dark:border-[#3a4d5e]/50">
          ...
          >
            {t('harness.mcp.copy.conflict.cancel', { defaultValue: 'Cancel' })}
          </button>
          <button
            type="button"
          ...
          >
            {t('harness.mcp.copy.conflict.submit', { defaultValue: 'Continue' })}
          </button>
        </div>
      </div>
    </div>
