...
vi.mock('../ThinkingBlock', () => ({
  ThinkingBlock: ({ content, isStreaming, provisional }: { content: string; isStreaming?: boolean; provisional?: boolean }) => (
    <div data-testid="mock-thinking-block" data-streaming={isStreaming}>{content}{provisional ? <span>잠정</span> : null}</div>
  ),
}));
...
describe('MessageArea', () => {
  ...
  // "Claude" / thinking header). Tests run under the ko locale (setup forces it), so the chip reads '잠정'.
